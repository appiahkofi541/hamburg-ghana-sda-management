...
type FinanceTab = "dashboard" | "accounts" | "transactions" | "reports" | "cash" | "bank" | "income" | "whatsapp";
...
type MemberOption = { id: string; name: string; whatsappPhone: string; phone: string };
...
export function FinanceManagement({ initialTab = "dashboard" }: { initialTab?: FinanceTab }) {
  ...
  useEffect(() => {
    async function loadFinance() {
      ...
      const [accountResult, categoryResult, memberResult, logResult, transactionResult] = await Promise.all([
        ...
        supabase.from("members").select("id, full_name, whatsapp_phone, phone").eq("status", "active").order("full_name"),
        supabase.from("whatsapp_payment_notification_logs").select("*").order("created_at", { ascending: false }),
        supabase
          .from("finance_transactions")
          .select("*, account:finance_accounts!finance_transactions_account_id_fkey(name), transfer_account:finance_accounts!finance_transactions_transfer_to_account_id_fkey(name), finance_categories(name), members(full_name)")
      ...
      setMembers((memberResult.data ?? []).map((row) => ({ id: row.id, name: row.full_name, whatsappPhone: row.whatsapp_phone ?? "", phone: row.phone ?? "" })));
  ...
  function openTransaction(transaction?: Transaction) {
    ...
    setTransactionForm(transaction ? {
      ...
      type: FINANCE_TRANSACTION_TYPES.includes(transaction.type as FinanceTransactionType) ? transaction.type as FinanceTransactionType : "Other",
  ...
  async function reloadAfterWrite() {
    ...
    const [{ data: accountRows }, { data: logRows }, { data: transactionRows }] = await Promise.all([
      supabase.from("finance_accounts").select("*").order("name"),
      supabase.from("whatsapp_payment_notification_logs").select("*").order("created_at", { ascending: false }),
      supabase.from("finance_transactions").select("*, account:finance_accounts!finance_transactions_account_id_fkey(name), transfer_account:finance_accounts!finance_transactions_transfer_to_account_id_fkey(name), finance_categories(name), members(full_name)").order("transaction_date", { ascending: false }),
    ]);
  ...
  async function saveTransaction(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (supabase) {
      ...
      const payload = {
        ...
        transaction_type: enumize(transactionForm.type),
  ...
  async function exportPdf() {
    ...
    autoTableModule.default(document, {
      startY: 24,
      head: [["Date", "Type", "Account", "Description", "Reference", "Amount"]],
      body: filteredTransactions.map((item) => [item.date, item.type, item.accountName, item.description, item.reference, currency.format(item.amount)]),
      styles: { fontSize: 8 },
  ...
  function exportExcel() {
    const headers = ["Date", "Type", "Account", "Transfer To", "Category", "Description", "Reference", "Amount"];
    const escapeXml = (value: string) => value.replace(/[<>&'"]/g, (character) => ({ "<": "&lt;", ">": "&gt;", "&": "&amp;", "'": "&apos;", '"': "&quot;" })[character]!);
    const row = (values: string[]) => `<Row>${values.map((value) => `<Cell><Data ss:Type="String">${escapeXml(value)}</Data></Cell>`).join("")}</Row>`;
    const rows = filteredTransactions.map((item) => row([item.date, item.type, item.accountName, item.transferToAccountName, item.categoryName, item.description, item.reference, String(item.amount)]));
