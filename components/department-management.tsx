...
const fallbackDepartmentNames = ["Elders", "Deacons", "Deaconesses", "Treasury", "Secretariat", "Sabbath School", "Youth Ministry", "Women's Ministry", "Men's Ministry", "Pathfinder Club", "Adventurer Club", "Choir", "Media Ministry", "Children's Ministry", "Personal Ministries", "Health & Temperance", "Singing Band"];
const seedRecords: DepartmentRecord[] = fallbackDepartmentNames.map((name, index) => ({
...
export function DepartmentManagement() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (supabase) {
        ...
        const [{ data, error: loadError }, { data: profileRows }, { data: membershipRows }] = await Promise.all([
        ...
        if (data?.length) {
          setRecords(data.map((row) => ({ id: row.id, name: row.name, description: row.description ?? "", leader: profileName(row.leader), leaderId: row.leader_id ?? "", meetingSchedule: row.meeting_schedule ?? "", memberCount: memberCounts.get(row.id) ?? 0, isActive: row.is_active })));
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (duplicate) { setError(`A department named "${form.name.trim()}" already exists.`); return; }
    setSaving(true);
    const supabase = createClient();
    let saved = { ...form, id: editing?.id ?? crypto.randomUUID() };
    if (supabase) {
      const payload = { name: form.name.trim(), description: form.description || null, meeting_schedule: form.meetingSchedule || null, leader_id: form.leaderId || null, is_active: form.isActive };
  ...
  async function toggleActive(record: DepartmentRecord) {
    ...
    const supabase = createClient();
    if (supabase) {
      const { error: updateError } = await supabase.from("departments").update({ is_active: !record.isActive }).eq("id", record.id);
