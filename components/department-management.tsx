  ...
  useEffect(() => {
    async function load() {
      ...
      if (supabase) {
        ...
        if (data?.length) {
          setRecords(data.map((row) => ({ id: row.id, name: row.name, description: row.description ?? "", leader: profileName(row.leader), leaderId: row.leader_id ?? "", meetingSchedule: row.meeting_schedule ?? "", memberCount: memberCounts.get(row.id) ?? 0, isActive: row.is_active, createdAt: row.created_at?.slice(0, 10) ?? "" })));
