...
type EventAccessMode = "member" | "leader";
...
export function CalendarManagement() {
  ...
  const [accessMode, setAccessMode] = useState<EventAccessMode>(createClient() ? "member" : "leader");
  ...
  const [eventCategories, setEventCategories] = useState<EventCategoryOption[]>(defaultEventCategories);
    ...
    async function loadEvents() {
      ...
      if (supabase) {
        ...
        if (user) {
          ...
          const memberOnlyUser = roleNames.length === 0 || roleNames.every((role) => role === "member");
          setAccessMode(memberOnlyUser ? "member" : "leader");
          ...
          if (member?.id && (member.profile_id === user.id || (!!loginEmail && member.email?.toLowerCase() === loginEmail.toLowerCase()))) {
            ...
            if (registrationError) {
              ...
              setRegistrations((registrationRows ?? []).map((row) => ({ id: row.id, eventId: row.event_id, status: registrationStatus(row.status ?? row.registration_status), confirmed: Boolean(row.attendance_confirmed) })));
            }
          } else {
            setMemberId("");
            setRegistrations([]);
            setMemberLookupMessage(memberOnlyUser ? `Member profile required. Link this login to public.members.profile_id = auth.users.id or set public.members.email to ${loginEmail || "the login email"}.` : "");
          }
          setMemberLookupComplete(true);
        } else {
          setMemberLookupComplete(true);
        }
        const [{ data: departmentRows }, { data: settingRows }, { data }, profile] = await Promise.all([
          supabase.from("departments").select("id, name, is_active").order("name"),
          supabase.from("record_settings").select("name, is_active").eq("setting_group", "event_category").order("sort_order").order("name"),
          supabase.from("events").select("*").order("starts_at"),
          loadPublicChurchProfile(supabase),
        ]);
        setChurchProfile(profile);
        if (settingRows?.length) {
          const tones = ["bg-blue-50 text-churchblue", "bg-amber-50 text-amber-700", "bg-emerald-50 text-emerald-700", "bg-purple-50 text-purple-700", "bg-slate-100 text-slate-600"];
          const activeSettings = settingRows.filter((row) => row.is_active);
          if (activeSettings.length) setEventCategories(activeSettings.map((row, index) => ({ name: row.name, tone: tones[index % tones.length], icon: defaultEventCategories[index % defaultEventCategories.length]?.icon ?? CalendarDays })));
        }
  ...
  function openForm(event?: CalendarEvent) {
    console.log("handleCreateEvent", { editingEventId: event?.id ?? null, canManage });
    if (!canManage) {
  ...
  async function saveEvent(submitEvent: React.FormEvent<HTMLFormElement>) {
    ...
    console.log("saveEvent submit", { editingEventId: editing?.id ?? null, form, canManage });
    if (!canManage) {
      setFormError("Access denied: only Super Admin/Admin or Secretary can save church events.");
      console.error("saveEvent blocked", { reason: "canManage=false" });
      return;
    }
      ...
      : !form.startsAt
        ...
        : form.recurrence !== "None" && !form.recurrenceUntil
          ...
          : form.endsAt && new Date(form.endsAt) < new Date(form.startsAt)
            ...
            : form.recurrenceUntil && new Date(form.recurrenceUntil) < new Date(form.startsAt)
              ? "Repeat until date must be on or after the start date."
              : "";
    if (validationError) {
      setFormError(validationError);
      console.error("saveEvent validation failed", { validationError, form });
      return;
    }
    setFormError("");
    setSavingEvent(true);
    ...
    try {
      if (supabase) {
        const payload = eventPayload(form, userId, Boolean(editing));
        console.log("saveEvent Supabase payload", { editingEventId: editing?.id ?? null, payload });
        const request = editing ? supabase.from("events").update(payload).eq("id", editing.id).select().single() : supabase.from("events").insert(payload).select().single();
        const { data, error } = await request;
        if (error) {
          console.error("saveEvent Supabase error", error);
          setFormError(`Unable to save event: ${error.message}`);
          return;
        }
        console.log("saveEvent Supabase success", data);
      ...
      console.error("saveEvent exception", error);
      setFormError(`Unable to save event: ${error instanceof Error ? error.message : "Unknown error"}`);
    } finally {
  ...
  async function registerForEvent(event: CalendarEvent) {
    ...
    if (!supabase) {
      ...
      setRegistrationFeedback("Unable to register: Supabase is not configured.");
    ...
    try {
      ...
      if (error) {
        ...
        setRegistrationFeedback(`Unable to register: ${error.message}`);
      ...
      setRegistrationFeedback(`Unable to register: ${error instanceof Error ? error.message : "Unknown error"}`);
  ...
  function handleRegisterClick(event: CalendarEvent) {
    console.log("Register button direct onClick", { eventId: event.id, memberId, eventTitle: event.title });
    setRegistrationFeedback("Register button clicked");
    setNotice("Register button clicked");
    window.setTimeout(() => {
  ...
  async function cancelRegistration(event: CalendarEvent) {
    ...
    const supabase = createClient();
    if (!supabase) return;
    const { error } = await supabase.from("event_registrations").update({ status: "cancelled", registration_status: "cancelled", attendance_confirmed: false, confirmed_at: null }).eq("id", registration.id);
  ...
  async function confirmAttendance(event: CalendarEvent) {
    ...
    if (!supabase) return;
    setRegisteringEventId(event.id);
    setRegistrationFeedback("Confirming attendance...");
    try {
      const resolvedEventId = await resolveEventRegistrationId(event, supabase);
      const confirmedAt = new Date().toISOString();
      const { error } = await supabase.from("event_registrations").update({ status: "attended", registration_status: "attended", attendance_confirmed: true, confirmed_at: confirmedAt }).eq("id", registration.id);
      if (error) { setNotice(`Unable to confirm attendance: ${error.message}`); setRegistrationFeedback(`Unable to confirm attendance: ${error.message}`); return; }
      const { error: attendanceError } = await supabase.from("event_attendance").upsert({
        event_id: resolvedEventId,
        member_id: memberId,
        present: true,
        checked_in_at: confirmedAt,
        checkin_method: "self_confirmed",
        notes: "Attendance confirmed by member from Events page.",
      }, { onConflict: "event_id,member_id" });
      if (attendanceError) {
        setNotice(`Attendance was confirmed, but My Attendance could not be synced: ${attendanceError.message}`);
        setRegistrationFeedback(`Attendance confirmed; sync failed: ${attendanceError.message}`);
        console.error("Unable to sync member event attendance", { eventId: resolvedEventId, memberId, attendanceError });
        return;
      }
    } catch (error) {
      console.error("Event attendance confirmation exception", error);
      const message = `Unable to confirm attendance: ${error instanceof Error ? error.message : "Unknown error"}`;
      setNotice(message);
      setRegistrationFeedback(message);
      return;
    } finally {
    ...
    setRegistrationFeedback("Attendance confirmed.");
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
        ...
        {canManage ? <Button onClick={() => openForm()}><Plus className="h-4 w-4" /> Create Event</Button> : <StatusBadge tone="slate">{accessMode === "leader" ? "Read-only" : "Member view"}</StatusBadge>}
      </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.25fr_1fr]">
        ...
        <Card className="p-5">
          <div className="flex justify-between gap-3"><div><h2 className="font-bold text-navy">Upcoming Events</h2><p className="mt-1 text-xs text-slate-400">Programs and activities</p></div>{canManage ? <Button size="sm" onClick={() => openForm()}><Plus className="h-4 w-4" /> Create Event</Button> : <StatusBadge tone="slate">Read-only</StatusBadge>}</div>
