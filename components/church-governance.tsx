...
const tabs = ["Nominating Committee", "Church Elections", "Church Board Voting", "Business Meeting Voting", "Meeting Minutes", "Election Reports"] as const;
...
export function ChurchGovernance() {
  ...
  const tabPolls = useMemo(() => {
    const map: Record<Tab, MeetingType | null> = { "Nominating Committee": "nominating_committee", "Church Elections": "church_election", "Church Board Voting": "church_board", "Business Meeting Voting": "business_meeting", "Meeting Minutes": null, "Election Reports": null };
...
function ReportsView({ rows, audit }: { rows: (string | number)[][]; audit: AuditRow[] }) {
  return <div className="grid gap-4 p-4"><Table title="Election & Voting Results" headers={["Title", "Module", "Kind", "Mode", "Status", "Votes", "Yes", "No", "Abstain", "Candidates"]} rows={rows} /><Table title="Governance Audit Trail" headers={["When", "Action", "Entity"]} rows={audit.map((row) => [row.createdAt.slice(0, 16).replace("T", " "), pretty(row.action), row.entityType])} /></div>;
}
