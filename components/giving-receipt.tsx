...
export function GivingReceipt({ id }: { id: string }) {
  ...
  useEffect(() => { async function load() { const supabase = createClient(); if (!supabase) return; const [{ data }, profile] = await Promise.all([supabase.from("online_giving_payments").select("*, funds(name)").eq("id", id).maybeSingle(), loadPublicChurchProfile(supabase)]); setPayment(data); setChurch(profile); } load(); }, [id]);
